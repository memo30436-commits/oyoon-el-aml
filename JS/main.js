...
.then(data => {
    // التقاط الضغطات من أي مكان في الموقع (الصفحة الرئيسية أو صفحة الأقسام)
    document.addEventListener("click", (event) => {
        const button = event.target.closest(".btn_add_cart, .add-to-cart-btn");
        ...
        if (button) {
            ...
            if (selectedProduct) {
                ...
                // تحديث شكل أزرار هذا المنتج في كل مكان بالموقع
                const allMatchingButtons = document.querySelectorAll(`.btn_add_cart[data-id="${productId}"], .add-to-cart-btn[data-id="${productId}"]`);
...
function updateCart() {
    ...
    cart.forEach((item , index) => {
        ...
        cartItemsContainer.innerHTML += `
            ...
            <div class="item_cart">
                ...
                <div class="content">
                    <h4>${item.name}</h4>
                    <p class="price_cart">${total_Price_item} ج.م</p>
    ...
    price_cart_total.innerHTML = `${total_Price} ج.م`
...
function updateButoonsState(productId) {
    ...
    allMatchingButtons.forEach(button => {
        ...
        button.innerHTML = `<i class="fa-solid fa-cart-shopping"></i> إضافة للعربة`;
    });
}
